...
function Popover({ coords, position = 'left', children, className = 'z-10' }: PopoverProps) {
  ...
  return createPortal(
    ...
    >
      <div className='absolute -top-[12px] right-0.5 h-0 w-0 border-l-[10px] border-b-[15px] border-r-[10px] border-l-transparent border-b-slate-200 border-r-transparent'></div>
      {children}
    </div>,
